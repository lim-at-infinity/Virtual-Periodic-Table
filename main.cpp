...
int main()
{
	...
	struct ElementInfo
	{
		...
		ElementInfo(const string& name, double weight, int number) : element_name(name), atomic_weight(weight), atomic_number(number) {}

	};
	
	typedef map<std::string, ElementInfo> PeriodicTable;
	...
	PeriodicTable::iterator it = pt.find("H");
	if (it != pt.end())
	{
		it->second;
	}
